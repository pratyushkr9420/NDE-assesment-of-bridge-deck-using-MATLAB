clear
load('24 0287L_half_cell_data_infobridge.mat')

start_loc = 1;
end_loc = size(half_cell_data, 2);
return
...
trisurf(tri, x_location(start_loc:end_loc), y_location(start_loc:end_loc), half_cell_data);
axis([0 265 0 40])
shading interp
colorbar
...
[X, Y, Z] = griddata(x_location(start_loc:end_loc), y_location(start_loc:end_loc), half_cell_data, xg, yg);
surf(X, Y, Z)
axis([0 265 0 40])
shading interp
colorbar
